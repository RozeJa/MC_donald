package cz.rozek.jan.admin_mc_donald.gui;
...
import cz.rozek.jan.admin_mc_donald.gui.dataPanesl.ADataPanel;
import cz.rozek.jan.admin_mc_donald.gui.forms.EditForm;
...
public class MainFrame extends JFrame {
    ...
    public void fillDataContent() {
        ...
        data = new TreeMap<>();
        try {
            data = (Map<String, IData>) db.getAll(actualClass);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    // pokud je aktuálně zobrazena nabýdka s obědnávkami, tak to přidá případně upravý @param order
    public void addOrderToPrint(Order order) {
        if (!actualClass.getName().equals(Order.class.getName())) {
            return;
        }
