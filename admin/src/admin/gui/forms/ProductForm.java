...
public class ProductForm extends EditForm<Product> { 
    ...
    private void loadImprevements() {
        Map<Integer, Improvement> improvements = new TreeMap<>();
        try {
            Map<Integer, Improvement> allImprovements = db.getAll(Improvement.class);

            if (data != null) {
                for (Improvement improvement : allImprovements.values()) {
                    if (improvement.getCategories().contains(data.getCategory())) {
    ...
    protected String setData() {
        ...
        if (products.values().contains(data) && data.getId() == 0) {
